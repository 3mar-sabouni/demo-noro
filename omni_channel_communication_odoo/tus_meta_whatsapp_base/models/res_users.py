...
class ResUsers(models.Model):
    ...
    def write(self, values):
        ...
        if values.get('groups_id'):
            ...
            uncheck_ids = []
            whatsapp_manager_group = self.env.ref('omni_channel_communication_odoo.whatsapp_group_manager').id
            inherited_groups = self.env['res.groups'].search(
                [('implied_ids', 'in', self.env.ref('omni_channel_communication_odoo.whatsapp_group_user').ids)])
            for group in groups:
