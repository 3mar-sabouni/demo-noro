        ...
        if message_obj.get('type') == 'image':
            ...
            attachment_value.update({'name': message_obj.get('document').get('filename'),
                                     ...
                                     'mimetype': message_obj.get('document').get('mime_type') if message_obj.get(
                                         'document') and message_obj.get('document').get('mime_type') else 'application/pdf'})
                    ...
                    for mes in data_value_obj.get('messages'):
                        ...
                        for partner in partners:
                            ...
                            if mes.get('type') == 'text':
                                ...
                                attachment, message_values, vals = self._get_received_attachment(mes, provider, message_values, vals)
                                message_values.update({
                            ...
                            if 'context' in mes or mes.get('reaction', {}).get('message_id', False):
                                parent_message = request.env['mail.message'].sudo().search_read(
                                    [('wa_message_id', '=', mes.get('context').get('id') if mes.get('context', False) else mes.get('reaction').get('message_id'))],
                            ...
                            request.env['whatsapp.history'].sudo().with_user(provider.user_id.id).with_context({'message': 'received'}).create(vals)
